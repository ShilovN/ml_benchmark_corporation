...
def mode_instruction(state: RunState) -> str:
    ...
    if mode == "single-shot":
        return (
            ...
            "Собери решение end-to-end: быстро изучи данные, создай код/модель/предсказания, проверь submission.csv "
            "и обязательно закончи командой submit(\"submission.csv\")."
        )
    if mode == "repeated":
        attempt = min(state.requests + 1, REPEATED_MAX_ATTEMPTS)
        final_note = (
            " Это последняя попытка: используй лучшее уже подготовленное решение и обязательно вызови submit(\"submission.csv\")."
            if attempt >= REPEATED_MAX_ATTEMPTS
            else " Не вызывай submit слишком рано, если еще есть очевидная проверка или улучшение; при готовом валидном решении можно отправлять."
        )
        return (
            ...
            "Каждый ответ должен давать конкретный прогресс: EDA, признаки, обучение, валидация или подготовка submission.csv."
            f"{final_note}"
        )
    if mode == "fixed-transitions":
        ...
        if stage == "EDA":
            detail = (
                "это этап 1/3. Изучи файлы, целевую колонку, размер данных, пропуски и базовые распределения. "
                "На этом этапе submit запрещен."
            )
        elif stage == "FEATURES":
            detail = (
                "это этап 2/3. Подготовь признаки, предобработку и скрипты для обучения/инференса. "
